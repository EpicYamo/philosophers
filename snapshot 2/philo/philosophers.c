...
static void	init_philosophers_pt_two(t_philo *philosophers, int philo_count)
{
	...
	if (philo_count != 1)
	{
		...
		i = 0;
		philosophers[0].philo_left = &philosophers[1];
		philosophers[0].philo_right = &philosophers[philo_count - 1];
		philosophers[philo_count - 1].philo_left = &philosophers[0];
		philosophers[philo_count - 1].philo_right = &philosophers[philo_count - 2];
		while (++i < (philo_count - 1))
		{
			philosophers[i].philo_left = &philosophers[i + 1];
			philosophers[i].philo_right = &philosophers[i - 1];
		}
	}
}
