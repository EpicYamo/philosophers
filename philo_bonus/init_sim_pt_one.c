...
long long	init_simulation(t_philo *philosophers, int philo_count)
{
	...
	if (init_sim_flag(philosophers, philo_count) != 0)
		return (LLONG_MAX - 1);
	if (init_fork_semaphore(philosophers, philo_count) != 0)
		return (LLONG_MAX - 2);
	if (init_print_semaphore(philosophers, philo_count) != 0)
		return (LLONG_MAX - 3);
	if (init_sim_semaphore(philosophers, philo_count) != 0)
		return (LLONG_MAX - 4);
	if (init_philo_semaphores(philosophers, philo_count) != 0)
		return (LLONG_MAX - 5);
